        ...
        <el-table :data="state.tableData" stripe style="width: 100%" v-loading="tableLoading" height="65vh" :element-loading-text="loadingText">
          ...
          <el-table-column prop="build" label="build版本" align="center" width="250">
            <template #default="scope">
              <!-- <span v-if="scope.row.build.split('-')[0] !== 'nodebug'">
                {{ scope.row.build.split('-')[0] === 'ArrayOS' ? scope.row.build.split('-')[1].split('.')[0] : scope.row.build.split('.')[0] }}
              </span>
              <span v-else>
                {{ scope.row.build.split('-')[0] === 'nodebug' ? scope.row.build.split('.')[0].replace('nodebug-', '') : scope.row.build.split('.')[0] }}
              </span> -->
              <span>{{ scope.row.build.split('.')[0].replace(scope.row.build.split('-')[0], '').replace('-', '') }}</span>
            </template>
          </el-table-column>
...
const handle = () => {
  state.tableData.map((item, index) => {
    ...
    item.uptimeAfter = utc2beijing(item.start_time) // '2022-09-16T17:44:08Z' => '2022/9/20 20:43:40'
