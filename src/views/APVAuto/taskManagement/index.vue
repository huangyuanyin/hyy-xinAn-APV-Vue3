<template>
  <div class="performanceManagement">
      ...
      <el-card class="box-card" shadow="never" v-loading="isStartingLoading" :element-loading-text="isLoadingText">
        <div class="search-wrap">
          ...
          <div class="ignore-select-wrap">
            ...
            <el-input size="large" clearable v-model="searchUser" placeholder="请输入要搜索的负责人..." :suffix-icon="Search" @change="getTask(1)" />
          </div>
        </div>
        ...
        <el-table :data="state.tableData" stripe style="width: 100%" v-loading="tableLoading" height="65vh" :element-loading-text="loadingText">
          ...
          <el-table-column prop="number" label="总数 / 执行 / 失败" align="center" width="180">
            <template #default="scope">
              ...
              <span style="margin: 0 5px">/</span>
              <span class="failNumStyle" @click="toDetail(scope.row.id)" v-if="scope.row.number && scope.row.number[1] != 0">{{ scope.row.number[1] }}</span>
            ...
            <template #default="scope">
              <div class="stateStyle" v-if="scope.row.state === 'waitcheck'">
                <el-tooltip content="继续运行该任务下失败用例" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #409eff" @click="changeTaskStatus('restart', scope.row, false)"><VideoPlay /></el-icon>
                </el-tooltip>
                <el-tooltip content="APV环境检测" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #f56c6c; margin-left: 10px" @click="checkAPV(scope.row.id)"><Warning /></el-icon>
                </el-tooltip>
                <el-tooltip content="测试环境释放" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #000; margin-left: 10px" @click="releaseAPV(scope.row.id)"><Promotion /></el-icon>
                </el-tooltip>
              </div>
              <div class="stateStyle" v-if="scope.row.state === 'stop'">
                <el-tooltip content="重新执行所有测试用例，并删除上次的测试结果" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #909399" @click="changeTaskStatus('start', scope.row, true)"><RefreshRight /></el-icon>
                </el-tooltip>
                <el-tooltip content="继续运行该任务下失败用例" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #409eff; margin-left: 10px" @click="changeTaskStatus('restart', scope.row, false)"><VideoPlay /></el-icon>
                </el-tooltip>
              </div>
              <div class="stateStyle fail" v-if="scope.row.state === 'fail'">
                <el-tooltip content="重新执行所有测试用例，并删除上次的测试结果" placement="top" effect="dark">
              ...
              <div class="stateStyle" v-if="scope.row.state === 'running'">
                <el-tooltip content="任务暂停" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #e6a23c" @click="changeTaskStatus('stop', scope.row, false)"><VideoPause /></el-icon>
                </el-tooltip>
              </div>
              <div class="stateStyle" v-if="scope.row.state === 'create'">
                <el-tooltip content="任务启动" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #67c23a" @click="changeTaskStatus('start', scope.row, false)"><SwitchButton /></el-icon>
                </el-tooltip>
              </div>
              <div class="stateStyle" v-if="scope.row.state === 'complete'">
                <el-tooltip content="继续运行该任务下失败用例" placement="top" effect="dark">
              ...
              <div class="stateStyle" v-if="scope.row.state === 'ready'">
                <el-tooltip content="任务准备中，禁止操作" placement="top" effect="dark">
                  <el-icon :size="16" style="color: #f56c6c; cursor: no-drop"><CircleClose /></el-icon>
                </el-tooltip>
              </div>
            </template>
          </el-table-column>
    ...
    <!--任务进度弹窗-->
    <el-dialog custom-class="taskProgressDialog" v-model="taskProgressDialog" title="任务进度" width="1050px" @close="closeTaskProgressDialog">
      <el-timeline>
    ...
    <!--确认预览弹窗-->
    <el-dialog v-model="submitPreviewDialog" title="添加任务预览" custom-class="submitPreviewDialog" width="50%" :close-on-click-modal="false">
